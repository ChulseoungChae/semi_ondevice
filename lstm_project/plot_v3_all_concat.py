"""V3 CNN_aug20x: Actual vs Predicted — all 45 CSV files, full timeline."""
...
from train_v3 import V3_INPUT_COLS, V3_INPUT_WINDOW, V3_PREDICT_AHEAD
...
MODEL_NAME = 'v3_CNN_aug20x'


def load_model_and_scalers():
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    ckpt = torch.load(os.path.join(MODEL_DIR, f'{MODEL_NAME}_best.pt'),
    ...
    with open(os.path.join(MODEL_DIR, f'{MODEL_NAME}_scaler_x.pkl'), 'rb') as f:
        scaler_x = pickle.load(f)
    with open(os.path.join(MODEL_DIR, f'{MODEL_NAME}_scaler_y.pkl'), 'rb') as f:
...
def create_windows_full(data, input_cols, output_col, input_window, predict_ahead):
    ...
    return np.array(idx_list), np.array(X_list)
...
def main():
    ...
    for f in files:
        ...
        recipe['data'] = pd.read_csv(f)
        recipe['total_rows'] = len(recipe['data'])
    ...
    all_actual_full, all_pred_full, all_active_mask = [], [], []
    segments = []
    offset = 0
    ...
    for d in all_files:
        ...
        idx, X = create_windows_full(df, V3_INPUT_COLS, 'PWPDS.Data',
                                     V3_INPUT_WINDOW, V3_PREDICT_AHEAD)
        if len(X) > 0:
            pred_full[idx] = predict_windows(X, model, scaler_x, scaler_y, device)
    ...
    act, prd = all_actual_full[valid], all_pred_full[valid]
    ...
    # Active-only metrics
    active_valid = valid & all_active_mask
    act_a, prd_a = all_actual_full[active_valid], all_pred_full[active_valid]
    mae_a = np.mean(np.abs(act_a - prd_a))
    rmse_a = np.sqrt(np.mean((act_a - prd_a) ** 2))
    m_a = np.abs(act_a) > 1e-6
    mape_a = np.mean(np.abs((act_a[m_a] - prd_a[m_a]) / act_a[m_a])) * 100

    # Idle-only metrics
    idle_valid = valid & ~all_active_mask
    act_i, prd_i = all_actual_full[idle_valid], all_pred_full[idle_valid]
    mae_i = np.mean(np.abs(act_i - prd_i))
    rmse_i = np.sqrt(np.mean((act_i - prd_i) ** 2))

    print(f"Total rows: {total:,}, Predicted: {valid.sum():,}")
    print(f"Global  — MAE={mae:,.0f}, RMSE={rmse:,.0f}, MAPE={mape:.4f}%")
    print(f"Active  — MAE={mae_a:,.0f}, RMSE={rmse_a:,.0f}, MAPE={mape_a:.4f}% (n={active_valid.sum():,})")
    print(f"Idle    — MAE={mae_i:,.0f}, RMSE={rmse_i:,.0f} (n={idle_valid.sum():,})")

    # ── Plot ──
    fig, axes = plt.subplots(3, 1, figsize=(30, 16),
                             gridspec_kw={'height_ratios': [4, 1.5, 0.6]})
    fig.suptitle(
        f'{MODEL_NAME}: Actual vs Predicted — All 45 Files, Full Timeline\n'
        f'Total {total:,} rows ({valid.sum():,} predicted)  |  '
        f'All: MAE={mae:,.0f} RMSE={rmse:,.0f} MAPE={mape:.4f}%  |  '
        f'Active: RMSE={rmse_a:,.0f}  Idle: RMSE={rmse_i:,.0f}',
    ...
    # --- Middle: Error ---
    ax2 = axes[1]
    error = all_actual_full - all_pred_full
    ...
    for grp in recipe_groups:
        ...
        ax3.text(mid, 0.5, grp['key'].replace('_', '\n'),
                 ha='center', va='center', fontsize=6.5, fontweight='bold')

    plt.tight_layout(rect=[0, 0, 1, 0.95])
    out_path = os.path.join(RESULT_DIR, f'{MODEL_NAME}_all_concat.png')
