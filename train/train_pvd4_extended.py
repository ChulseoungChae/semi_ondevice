...
class DataProcessor:
    ...
    def load_csv_files(self):
        ...
        print(f"[PVD4] Skipped {missing_cols_count} files (missing target columns)")
        print(f"[PVD4] Target columns: {self.target_columns}")
        return dataframes
    ...
    def prepare_data(self):
        ...
        print(f"[PVD4] Input shape: {sequences.shape}, Target shape: {targets.shape}")
    ...
    for epoch in range(config.EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'val_mae': val_mae,
                'epoch': epoch,
            }, os.path.join(save_dir, f'{model_name}_aug100_extended_best.pth'))
        else:
            patience_counter += 1
...
def main():
    ...
    # 데이터 준비
    print("\n" + "="*60)
    print("PVD4 Extended Model Training")
    print("Targets: 7 columns (4 original + 3 new)")
    print("Augmentation: 100x")
    print("="*60)
    ...
    os.makedirs(save_dir, exist_ok=True)
    processor.save_scalers(save_dir, '_aug100_extended')

    # ==================== LSTM 학습 ====================
    print("\n" + "#"*60)
    print("# LSTM Model (Extended - 7 targets)")
    print("#"*60)
    ...
    lstm_val_loss, lstm_val_mae, lstm_best_epoch = train_model(
        lstm_model, train_loader, val_loader, config, device, 'lstm', save_dir
    )
    lstm_time = time.time() - start_time

    # Best 모델 로드 및 평가
    checkpoint = torch.load(os.path.join(save_dir, 'lstm_aug100_extended_best.pth'))
    lstm_model.load_state_dict(checkpoint['model_state_dict'])
    lstm_results = evaluate_model(lstm_model, val_loader, processor.scaler_target, device, config.PVD_TARGETS['PVD4'])

    # ==================== 결과 출력 ====================
    print("\n" + "="*70)
    print(" TRAINING RESULTS (Augmentation = 100, 7 Targets)")
    print("="*70)

    print(f"\nOverall:")
    print(f"  Val Loss: {lstm_val_loss:.6f}")
    print(f"  Val MAE:  {lstm_val_mae:.6f}")
    print(f"  Best Epoch: {lstm_best_epoch + 1}")
    ...
    # 결과 파일 저장
    with open(os.path.join(save_dir, 'lstm_aug100_extended_results.txt'), 'w') as f:
        f.write("PVD4 Extended LSTM Training Results\n")
        f.write("="*70 + "\n")
        f.write("Augmentation: 100x\n")
        f.write(f"Target Columns: {config.PVD_TARGETS['PVD4']}\n\n")

        f.write("Overall Performance:\n")
        f.write(f"  Val Loss: {lstm_val_loss:.6f}\n")
        f.write(f"  Val MAE:  {lstm_val_mae:.6f}\n")
        f.write(f"  Best Epoch: {lstm_best_epoch + 1}\n")
        f.write(f"  Train Time: {lstm_time:.1f}s\n\n")
    ...
    print(f"\nResults saved to: {os.path.join(save_dir, 'lstm_aug100_extended_results.txt')}")
    print(f"Model saved to: {os.path.join(save_dir, 'lstm_aug100_extended_best.pth')}")
